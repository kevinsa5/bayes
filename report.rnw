...
\title{Constructing Bayesian Models for Nuclear Astrophysics }
...
\section{Creating Models with rjags}
...
<<fig.pos="H", fig.height=4, fig.width=6>>=
plot(x,y)
@
...
\todo{how does the posterior plot vary with higher N, bigger sigma than slope, etc}
